...
class Sampler_Multiple_Samples(EnvExperiment):
    ...
    def run(self):
        ...
        for i in range(n_channels):                                                     #loops for each sampler channel 
            ...
            delay(100*us)                                           #sets each channel's gain to 0db
            
        smp = [0.000]*n_channels                                                            #creates list of 8 floating point variables
        
        for n in range(n_samples):                                                      #loops over number of samples do be taken
        
            delay(200*us)                                                                #90us delay to prevent uderflow in sampling stage                                   
            self.sampler0.sample_mu(smp)
            delay(10000*us)                                                #runs sampler and saves to list 
            self.mutate_dataset("samples",n,smp[0])
            delay(100*us)                                     #adds 0th element of list to dataset so voltage from lowest channel being used
