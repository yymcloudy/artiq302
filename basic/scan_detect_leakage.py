...
class BaseSequence_Detection_Leakage(TrapEnvScan):
    """detection leakage scan BaseSequence"""
    num_shots = 0
    def build_fragment(self):
        ...
        self.setattr_param("illumination_time", FloatParam, "Illumination time", default=500.0*us, unit="us")
        # self.setattr_param("illumination_amplitude", FloatParam, "Illumination amplitude", default=0.11, unit="dB")
    ...
    def illumination(self):
        with parallel:
            self.double_pass_370.set(frequency=self.double_pass_frequency.get(),phase=0.0, amplitude = 0.08)
            self.laser370_sideband_control(sideband='14.7', enable=False)
            self.laser370_sideband_control(sideband='2.1', enable=False)
        self.laser370_switch_control(switch='on')
        delay(self.illumination_time.get())
        self.laser370_switch_control(switch='off')
...
BaseSequenceExp = make_fragment_scan_exp(BaseSequence_Detection_Leakage)
