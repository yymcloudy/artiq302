...
class BaseSequence_SpinEcho(TrapEnvScan):
    """spin echo mw Sequence"""
    num_shots = 0
    def build_fragment(self):
        ...
        self.setattr_param("mw_duration_pi", FloatParam, "Ramsey pi MW duration", default=300.0*us, unit="us")
        self.setattr_param("mw_phase", FloatParam, "phase between 2 pulses", default=0, min=0, max=1)
        self.setattr_param("mw_phase_echo", FloatParam, "phase of echo pulse", default=180, min=0, max=1)
    ...
    def mw_gate(self):
        ...
        with parallel:
            self.mw_switch.on()
        delay(self.mw_duration_pi.get()/2)
        self.mw_switch.off()
...
BaseSequenceExp = make_fragment_scan_exp(BaseSequence_SpinEcho)
