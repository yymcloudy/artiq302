...
class BaseSequence_Rabi(TrapEnvScan):
    """rabi mw scan BaseSequence copy"""
    num_shots = 0
    ...
    def prepare(self):
        ...
        self.double_pass_370.set(frequency=self.double_pass_frequency.get(), phase=0.0, amplitude = 0.2)
        self.double_pass_370.sw.on()

        # self.mw_tunefreq.set(self.mw_freq.get())
        self.mw_tunefreq.set(frequency=self.mw_freq.get(), phase=0.0,amplitude = 0.6)
    ...
    def mw_gate(self):
        ...
        self.mw_tunefreq.set(frequency=self.mw_freq.get(),phase=0.0, amplitude = 0.8) # to do: change dds source
        with parallel:
            self.mw_switch.on()
