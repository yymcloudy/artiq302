...
class tbj(TrapEnvScan):
    ...
    def mw_gate(self):
        ...
        self.mw_tunefreq.set(self.mw_freq.get()) # to do: change dds source
        with parallel:
            self.mw_switch.on()
    ...
    def detection(self):
        ...
        self._push_counts([self.cooling_time.get(), self.pumping_time.get(), self.detection_time.get(), float(num)])
    ...
    @kernel
    def run_once(self):
        # ttl_test: measure instrument, sampler input
        # dds_test: measure instrument
        # duration: keep time + sampling time + dds set time
        self.core.reset()
        self.core.break_realtime()
        self.dds_test.set_att(0*dB)
        self.dds_test.set(frequency=180*MHz, amplitude=0.0)
        self.dds_test.sw.on()
        data=[0.000]
        # ignore the ttl open or close duration
        self.ttl_test.off()
        self.adc.sample(data)
        # if data[0]<0.5:
        delay(100*ms)
        self.dds_test.set(frequency=180*MHz, amplitude=0.4)
        delay(100*ms)
        self.dds_test.set(frequency=180*MHz, amplitude=0.8)
        delay(100*ms)
        self.ttl_test.on()
        delay(300*ms)
        # print(data)
...
tbjExp = make_fragment_scan_exp(tbj)
